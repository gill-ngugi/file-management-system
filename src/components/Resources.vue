<template>    
    <div>
        ...
        <div class = "nav-menu" style="height:45px; width:100%; padding:0px; padding-top:0.3%; text-align:center;">
            ...
            <router-link to="/" style="color:#fff; font-size:19px;"><v-icon color="#fff" size="35" style="margin-left:10px;">mdi-home</v-icon>Home</router-link>
            <p style="font-weight:bold; font-size:24px; margin-left:40%;">Resources</p>
        </div>

        <!-- COMPONENT -->
              <div style="height:850px; margin-top:0.5%; margin-bottom:0.5%; background-color:#f8f9f9; border-top:5px solid rgb(227,58,58); width:98%; margin-left:1%; margin-right:1%;">
                <div class = "parent" style="height:45px; width:100%; margin-top: 7px; background-color:#fff;">
                    <div class="left-buttons">                
                        <!-- <router-link to=""><v-icon v-on:click="loadItemFromHistory()" style="float:left; margin-left:10px; background-color:rgb(227,58,58); color:#fff" size="30">mdi-chevron-left</v-icon></router-link>  -->
                        <v-icon style="float:left; margin-left:10px; background-color:rgb(227,58,58); color:#fff" size="30">mdi-chevron-right</v-icon>
            ...
            <!-- Back History Header-->
            <button v-on:click="reloadPage()" style="background-color:rgb(227,58,58); padding:10px; color:#fff; font-weight:bolder; font-size:19px; border-radius:20px;">
               Display All Folders                 
            </button>
        ...
        <!--  VUE ARRAY SORTING EXAMPLE             
                ...
                <div class="blackish" style="width:100%; margin-top:20px;">
        ...
        <div class = "nav-menu" style="height:45px; width:100%; padding:0px; padding-top:0.3%; padding-left:40%;">
            ...
            <p style="font-weight:bold; font-size:19px;">File Management System</p>
        </div>
    </div>       
</template>
    ...
    export default{
        ...
        methods: {
            ...
            loadItemFromHistory(){
                ...
                this.getRecentDocuments=currentTop;
            ...
            // },
               ...
               getRecentDocumentsZ(){
                axios.get("../assets/json-APIs/0-getMeetingPackFolder.json")
                    .then(response => {
                        this.getRecentDocuments =response.data.data.itemSubArray;
                        this.parentname=response.data.data.itemName;
                //this.navigationPath.push({"parentname":response.data.data.itemName,"itemSubArray":this.getRecentDocuments})
                    
                       // this.$localStorage.set('getRecentDocuments', JSON.stringify(this.getRecentDocuments))
                       // console.log(response.data.data.itemSubArray);
                    })
                    .catch(e => {
                        console.log('Error', e);
                    })
            ...
            getResourcesFilesZ(){                
                axios.get("../assets/json-APIs/getResourcesFiles.json")
                    .then(response => {
                        this.getResourcesFiles = response.data.data.itemSubArray;
            ...
            getUserData(itemId=0,parentItemId=0){
              ...
              getMeetingPack(item){
                  //alert(item.itemSubArray.length)
                  
                 // console.log("children from p",item.itemSubArray)
                //  this.navigationPath.push({"parentname":this.parentname,"itemSubArray":this.getResourcesFiles})
                   ...
                   if(item.hasOwnProperty("itemExtension")){
                       ...
                       this.parentname=item.itemName;
                      this.getResourcesFiles = item.itemSubArray;
                    //   this.getResourcesFiles = item.itemSubArray
                   }
        ...
        beforeMount(){
            ...
            this.getRecentDocumentsZ();
            this.getResourcesFilesZ();
